...
tasks.jar {
    manifest {
        attributes(
            mapOf(
                "Implementation-Title" to project.name, "Main-Class" to application.mainClass.get()
            )
        )
    }
...
val Process.txt
    get() = inputStream.reader().readText()
...
task("clearDocker") {
    doLast {
        ...
        val b = a.substringAfter("NAMES").substring(1).substringBefore(' ')
